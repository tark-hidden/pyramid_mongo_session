...
def data(accessed, created, value, to_pickle=False):
    return dict(
        accessed=accessed,
        created=created,
        value=value,
...
class CookieSessionTests(Serializer):
    ...
    def test_ctor_with_cookie_still_valid(self):
        request = testing.DummyRequest()
        cookieval = self._serialize(data(time.time(), 0, {'state': 1}))
    ...
    def test_ctor_with_cookie_expired(self):
        request = testing.DummyRequest()
        cookieval = self._serialize(data(0, 0, {'state': 1}))
        request.cookies['session'] = cookieval
        session = self._makeOne(request)
        self.assertEqual(dict(session), {})
    ...
    def test_timeout(self):
        request = testing.DummyRequest()
        cookieval = self._serialize(data(time.time() - 5, 0, {'state': 1}))
        request.cookies['session'] = cookieval
    ...
    def test_timeout_never(self):
        ...
        cookieval = self._serialize(
            data(time.time() + LONG_TIME, 0, {'state': 1}))
        request.cookies['session'] = cookieval
    ...
    def test_reissue_triggered(self):
        request = testing.DummyRequest()
        cookieval = self._serialize(data(time.time() - 2, 0, {'state': 1}))
        request.cookies['session'] = cookieval
        session = self._makeOne(request)
...
class IssueSessionTests(CookieSessionTests, unittest.TestCase):
    def _makeOne(self, request, **kw):
        ...
        return MongoSessionFactory(collection, **kw)(request)
...
class TestPickedSession(PickleSerializer, unittest.TestCase):
    def _makeOne(self, request, **kw):
        from pyramid_mongo_session import MongoSessionFactory
        return MongoSessionFactory(collection, True, **kw)(request)

    def test_pickled_object(self):
        request = testing.DummyRequest()
        cookieval = self._serialize(data(time.time(), 0, {'state': 1}, True))
        request.cookies['session'] = cookieval
        session = self._makeOne(request)
        self.assertEqual(session['state'], 1)
        self.assertFalse(session._dirty)
